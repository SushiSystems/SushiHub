...
{
public:
    /** @brief Binds the workspace to the `ss` executable every run is spawned from. */
    ...
    /** @brief Returns the `ss` executable every run is spawned from. */
    const std::string& executable() const;

private:
    /** @brief Builds the argument vector that asks `ss` for @p screen as JSON. */
    ...
    /** @brief Holds the `ss` executable, however the caller found it. */
