...
**Status:** designed 2026-09-21. Waves 0 through 5 landed 2026-09-22; waves 6, 7 and 8 are open.
...
The desktop application is distributed separately, decided by the owner on 2026-09-22. It goes to
PyPI as its own `sushihub-gui` distribution with a wheel per platform, reached as
`pipx install "sushihub[gui]"`, and it is versioned from the same tag as `sushihub` so the two
never disagree about the `--describe` contract. `sushihub` itself stays `py3-none-any`: a
...
| 6 | A reader for `sushi-module.toml` and the file in each of the four modules. The catalog becomes the fallback; `hub add <git-url>` stays out. Plan: `../agent/plans/2026-09-22-wave-6-a-module-describes-itself.md` | 2 | A checkout carrying a manifest is recognised with no catalog entry |
| 7 | `sd setup` and `st setup` provision their own repositories: they hand off to `hub install` inside a workspace and read their own fragment outside one. The fragment reader moves into `sushicore`, so one schema has one reader. Plan: `../agent/plans/2026-09-22-wave-7-sd-and-st-stand-on-their-own.md` | 2, 6 | Both repositories stand on their own READMEs, `sd build` and `st build` work, and `hub doctor` counts a `sushidsp` owner when it is linked |
