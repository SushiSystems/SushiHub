...
| 1a | Move `cli/` to `sushihub/cli/`; define the JSON contract and its schema under `sushihub/contract/`; add `JsonRenderer` to `sushicore`; add `ss --describe`; give every existing command `--json`. | 0 |
| 1b | Dependencies follow the modules: `build_pipeline` derives the toolchain selection from the present modules' fragments; `ss add` provisions what the new module needs; `ss doctor` reports by owner in dependency order; the install scripts stop running `ss install` before any module exists. | 0 |
| 2 | Binary presence: the third form in `modules.local.toml` and `ss status`; the binary-root marker in `ModuleProfile`; `ss login`, `ss logout`, `ss whoami` against a fake Sushi ID. | 1a |
...
| 4 | `sushihub/gui/`: the Dear ImGui application, the `ss` subprocess bridge over the JSON contract, hand-drawn screens for status, modules, dependencies, licence and projects, generated forms for the rest. | 1a; 2 in parallel |
